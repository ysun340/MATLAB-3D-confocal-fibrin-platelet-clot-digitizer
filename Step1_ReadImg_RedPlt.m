...
% This script setup reads the platelets in red. The output workspace of
...
clear;
clc;
cutoff = 60;   % used to control detection of intensity/color
dataIndex = 0;
depth = 201;  % identify the images need to be read

for z = (depth+1):(2.*depth)
    
    originalImage = imread('2k-center_3.tif',z);
    originalImage = originalImage(400:end-220,200:end-220,:);  % discard the outer range of the image which has bad quality, vary based on img
...
for i = 1:(length(x(1,:))-1)
    ...
    for j = 1:length(x(:,1))
        ...
        for k = 1:length(x(:,1))
            ...
            if (x(j,i) - x(k,i+1)).^2 + (y(j,i) - y(k,i+1)).^2 < (r(j,i)./2).^2
